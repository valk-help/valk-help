/* script.js
   - snow canvas
   - small UI wiring for the Open buttons
*/
...
(function(){
  ...
  function loop(now){
    ...
    for(let i=0;i<flakes.length;i++){ flakes[i].update(delta); flakes[i].draw(ctx); }
    requestAnimationFrame(loop);
  }
  requestAnimationFrame(loop);
})();

// tiny button wiring: open links when "Open" clicked
document.addEventListener('click', function(e){
  const t = e.target;
  if(t && t.classList && t.classList.contains('btn-round') && t.textContent.trim().toLowerCase() === 'open'){
